    ...
    <footer className="bg-obsidian-deep border-t border-border-highlight">
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <p className="font-body text-on-surface-variant text-sm leading-relaxed max-w-xs">
              Dubai&apos;s premier destination for paint protection, ceramic
              coatings, and custom vehicle transformations. Protecting what
              drives you.
            </p>
        ...
        <div className="mt-16 pt-8 border-t border-border-highlight flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          {/* Legal links repeated inline */}
          <div className="flex items-center gap-5">
            {legalLinks.map(({ label, to }) => (
              <Link
                key={label}
                to={to}
                className="font-mono text-[10px] text-outline uppercase tracking-widest hover:text-primary transition-colors duration-200"
              >
                {label}
              </Link>
            ))}
